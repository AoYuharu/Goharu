...
def render_step_result(step_number, action):
    """只渲染工具结果（工具调用已经在on_tool_call_start中显示）"""
    action_type = action.get("type")
    ...
    if action_type == "tool_batch":
        ...
        if RICH_AVAILABLE and console:
            ...
            for tc in tool_calls:
                console.print(Panel(
                    escape(tc["result_preview"]),
                    title=f"[bold green]✓ Tool Result: {escape(tc['tool_name'])}[/bold green]",
                    border_style="green",
                    padding=(1, 2)
                ))
        else:
...
async def run_agent(
    ...
    if RICH_AVAILABLE and console:
        ...
        for step in range(max_depth):
            ...
            # 只渲染工具结果（工具调用已经在on_tool_call_start中显示了）
...
async def main():
    ...
    try:
        ...
        # 设置子agent输出回调
        from Tools.builtin.agent_delegate import _manager as agent_manager
        ...
        while True:
            ...
            if RICH_AVAILABLE and console:
                from rich.markup import escape
                console.print(Panel(escape(result["final_answer"]), title="[bold green]Assistant[/bold green]", border_style="green"))
